...
signupForm.addEventListener('submit', (e) => {
  ...
  if (!passwordPattern.test(password)) {
    document.getElementById('password-error').textContent = "كلمة المرور يجب أن تحتوي على 8 أحرف على الأقل وتشمل حروفًا كبيرة وصغيرة وأرقامًا.";
    document.getElementById('password-error').classList.remove('hidden');
    isValid = false;
  } else {
    document.getElementById('password-error').classList.add('hidden');
  ...
  if (!phoneNumberPattern.test(phoneNumber)) {
    document.getElementById('phoneNumber-error').textContent = "الرجاء إدخال رقم هاتف صحيح (يبدأ بـ 07 ويحتوي على 10 أرقام).";
    document.getElementById('phoneNumber-error').classList.remove('hidden');
    isValid = false;
  } else {
    document.getElementById('phoneNumber-error').classList.add('hidden');
  ...
  if (!firstname) {
    document.getElementById('firstname-error').textContent = "الرجاء إدخال الاسم الأول.";
    document.getElementById('firstname-error').classList.remove('hidden');
    isValid = false;
  } else {
    document.getElementById('firstname-error').classList.add('hidden');
  ...
  if (!lastname) {
    document.getElementById('lastname-error').textContent = "الرجاء إدخال اسم العائلة.";
    document.getElementById('lastname-error').classList.remove('hidden');
    isValid = false;
  } else {
    document.getElementById('lastname-error').classList.add('hidden');
...
signinForm.addEventListener('submit', (e) => {
  ...
  if (!emailPattern.test(email)) {
    document.getElementById('signin-email-error').textContent = "الرجاء إدخال بريد إلكتروني صحيح.";
    document.getElementById('signin-email-error').classList.remove('hidden');
    isValid = false;
  } else {
    document.getElementById('signin-email-error').classList.add('hidden');
  ...
  if (!password) {
    document.getElementById('signin-password-error').textContent = "الرجاء إدخال كلمة المرور.";
    document.getElementById('signin-password-error').classList.remove('hidden');
    isValid = false;
  } else {
    document.getElementById('signin-password-error').classList.add('hidden');
...
// Google Sign In/Up
document.getElementById('google-signup').addEventListener('click', () => {
  showToast("تسجيل الدخول عبر جوجل غير متاح حالياً.", false);
});
